...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  4 16:03:44 2021

@author: taterosen

twitter api example p331
"""
...
# CREATE NEW FILE
def open_file(api, fromDate, endDate):
    csvFile = open('result.csv','w',encoding='utf-8')
    csv.writer(csvFile).writerow(["created", "text", "retwc", "hashtag", "followers", "friends"])
    tw_search(api, csvFile, fromDate, endDate)
    csvFile.close()
    return csvFile
    
def addToFile(api, file, fromDate, endDate):
    with open('result.csv', 'a') as csv:
        tw_search(api, csv, fromDate, endDate)


# TWEEPY SEARCH FUNCTION
def tw_search(api, file, fromDate, endDate):
    counter = 0
    ...
    for tweet in tweepy.Cursor(api.search,
                                q = qw,
                                since = fromDate, #"2021-06-07",
                                until = endDate, #"2021-06-08",
                                g = ge,
                                lang = l,
                                result_type = t,
                                count = c).items():
...
def main():
    ...
    tw_parser()
    api = tw_oauth(authfile)
    file = open_file(api, "2021-06-10_15:54:13", "2021-06-10_16:12:23")
    addToFile(api, file, "2021-06-10 15:34:13", "2021-06-10 15:54:13")
    addToFile(api, file, "2021-06-08", "2021-06-09")
    addToFile(api, file, "2021-06-07", "2021-06-08")
    ###tw_search(api)
